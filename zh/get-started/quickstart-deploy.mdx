...
- 具备访问 `docker.io`(或镜像源)的**出站网络**,以拉取相关镜像。
...
<Steps>
  ...
  <Step title="拉起技术栈">
    ...
    每 5 秒打印一个 `.`——首次启动时冷 MySQL 初始化可能耗时 60–90 秒。
  ...
  <Step title="验证部署">
    ...
    - **`[infra]`** — 容器健康、nginx 路由、`octo-server` REST、matter、MinIO 健康,以及
      管理台 / Web 两个 SPA。
...
所有客户端流量经 nginx 从 `28080` 进入;每个后端服务都绑定在回环地址上。
...
<CardGroup cols={2}>
  ...
  <Card title="Deploy Octo" icon="github" href="https://github.com/Mininglamp-OSS/octo-deployment">
    官方部署仓库(Compose + Kubernetes)。
  </Card>
</CardGroup>
