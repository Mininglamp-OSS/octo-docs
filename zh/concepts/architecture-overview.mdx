...
graph TD
  ...
  subgraph Core
    ...
    fleet[octo-fleet<br/>bot/daemon orchestration]
    matter[octo-matter<br/>tasks]
...
`octo-server` 组织为 `modules/` 下的 **27+ 个自动注册模块**（每个模块通过 `init()` + `register.AddModule()` 自行接线），而非单一巨石。机器人身份、代理执行（on-behalf-of）编排、Space、话题以及 BotFather 各自独立成模块。运行时与机器人集群（bot-fleet）编排正在被抽取到 [`octo-fleet`](/zh/ecosystem/repository-guide) 中，使 IM 核心能够按自己的节奏演进。
