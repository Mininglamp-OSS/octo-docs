...
当智能体执行某个动作时，结果通过**交互式卡片协议**（Adaptive Cards 1.5，`octo/v1` 配置文件）呈现，使工具调用的预览与操作在客户端中原生渲染——也就是你在 [octo-web](/zh/guides/teams/use-chat-docs-tasks) 中看到的“内联工具调用预览”。

## 集群信息归属何处

哪些 Daemon 存活、它们承载哪些机器人，以及各供应商运行时（Claude Code、Codex、OpenClaw、Hermes）运行在何处，都由 **[`octo-fleet`](/zh/ecosystem/repository-guide)** 拥有——它从 `octo-server` 中拆分出来，使 IM 核心卸下智能体集群相关的负担。机器人令牌从不离开服务端；浏览器只能看到一个 `bot_uid`，而 Daemon 则用其受限作用域的 JWT 去获取令牌。
