---
title: "发布到应用市场"
description: "Skills 与 MCP 服务器发布的地方——octo-marketplace 控制平面。"
icon: "store"
---

**[`octo-marketplace`](https://github.com/Mininglamp-OSS/octo-marketplace)** 是 Octo
...
<Warning>
  **逐步成熟的服务。** Skill 生命周期(上传 / 解析 / 创建 / 版本 / 下载、分类、标签)与
  MCP 目录(CRUD、探测、图标、分类)已在当前规范与代码中实现,Skill 与 MCP 记录均由 MySQL 持久化。
  目录发现 UI 与发布/版本化的细节完善仍在继续——请将生产使用视为尚处早期。
</Warning>
...
[`octo-smart-summary`](/zh/ecosystem/repository-guide) 的 API 服务形态。认证是统一的用户 + `bf_` 助理，
...
<Card title="今天就编写一个 Skill" icon="puzzle" href="/zh/guides/bot-developers/publish-a-skill">
  技能现在就可以通过 git 原生的安装路径使用——应用市场在此之上加入发现、版本化与分发能力。
</Card>
