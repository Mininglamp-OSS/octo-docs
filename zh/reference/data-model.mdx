---
title: "数据模型与实体"
description: "Octo 的核心实体——Space、User、Bot、Channel、Message、Thread——及其关键属性、枚举与关系。"
...
Bot 是用户的一种特化,而非独立的身份层。Channel 是 WuKongIM 概念,以一个 id 加一个 `channel_type` 标识;
...
规范化的身份。`uid` 是被空间成员、群成员、消息发送者与 Bot 所引用的通用键。定义于
...
Bot 是一个用户(`robot=1`),由两类携带凭证的记录之一作为前端。身份解析按**令牌前缀**路由
(`app_*` → App Bot,`bf_*` → User Bot),位于 `octo-server/modules/botidentity`。
...
| 属性 | 含义 |
|---|---|
| `robot_id` · `username` | Bot 身份(`username` 以 `_bot` 结尾)。 |
...
<AccordionGroup>
  ...
  <Accordion title="Integration 与 incoming webhook — octo-server/modules/integration, incomingwebhook">
    ...
    `iwh_` 前缀的合成发送者身份投递(`incomingwebhook`)。这就是外部系统在不作为完整 Bot 的情况下
    向频道发送消息的方式。
  </Accordion>
