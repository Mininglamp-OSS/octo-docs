...
sibling services all talk to it; it orchestrates business logic and agent scheduling and drives
[WuKongIM](/concepts/messaging-and-im-core) for real-time messaging.

```mermaid
graph LR
  clients["Clients<br/>web · mobile · extension · admin"] --> nginx["nginx :28080"]
  nginx --> server["octo-server<br/>orchestration · agent scheduling"]
  nginx -->|/ws| im["WuKongIM"]
  server --> im
  server --> svc["Core services<br/>smart-summary · speech<br/>docs · marketplace"]
  server --> data[("MySQL · Redis · MinIO")]
  agents["Agent plane<br/>channels · octo-cli · octo-daemon"] --> server
  server -. optional .-> search["Search & export<br/>Kafka · OpenSearch"]
  server -. uses .-> shared["octo-lib · octo-auth · octo-adapters"]
```
...
<Steps>
  ...
  <Step title="Execute">Run business logic, possibly spawning or resuming an agent session.</Step>
...
<CardGroup cols={2}>
  ...
  <Card title="Repository guide" icon="diagram-project" href="/ecosystem/repository-guide">
    All the repos and how they connect.
  </Card>
</CardGroup>
