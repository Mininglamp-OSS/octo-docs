...
description: "The core Octo entities — Space, User, Agent, Channel, Message, Thread — with their key attributes, enums, and relationships."
...
Agents are a specialization of users, not a separate identity plane. Channels are a WuKongIM concept
...
message senders, and agents. Defined in `octo-server/modules/user/db.go` (`user` table).
...
| `robot` | `1` when this user is an agent. |
...
An **Agent** is a user (`robot=1`) fronted by one of two credential-bearing records. Identity
resolution routes by **token prefix** (`app_*` → Intelligent Application, `bf_*` → Assistant) in
...
| Attribute | Meaning |
|---|---|
| `robot_id` · `username` | Agent identity (`username` ends in `_bot`). |
...
An **Expert** is a specialized Agent — an Assistant curated for a particular domain or task (its own
instructions, skills, and tools). It is a **product-level specialization, not a distinct backing
table**: on the wire an Expert is an Assistant (`bf_…`, the `robot` record above).
...
<AccordionGroup>
  ...
  <Accordion title="Integration & incoming webhook — octo-server/modules/integration, incomingwebhook">
    ...
    how outside systems send into channels without being full agents.
  </Accordion>
...
|---|---|
| `bf_` | Assistant token |
| `app_` | Intelligent Application token |
